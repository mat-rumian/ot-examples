...
main () {
    ...
    if [[ $deploy == true ]]; then
        echo "sam deploying..."
        sam deploy --stack-name $stack --region $region --capabilities CAPABILITY_NAMED_IAM --resolve-s3 --parameter-overrides ParameterKey=SumoHttpTracesUrl,ParameterValue=${SUMO_HTTP_TRACES_URL}
    fi
}
